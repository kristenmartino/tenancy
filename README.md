...
- **Sample output:** [`docs/sample-abstract.pdf`](docs/sample-abstract.pdf) — a real lease abstraction in PDF form, generated by opening any lease detail page → Cmd+P → "Save as PDF". Print stylesheet hides chrome (PDF viewer, Q&A, resolve buttons) and renders the extraction + exceptions read-only.
